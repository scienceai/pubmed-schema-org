...
exports.journal = function($journalMeta){
  if(! $journalMeta) return;
  ...
  if($issn){
    journal.issn = exports.issn($issn);
  }
...
exports.inline = function($article){
  if(!$article) return;
...
exports.volume = function($articleMeta){
  if(! $articleMeta) return;

  var $volume = $articleMeta.getElementsByTagName('volume')[0];
  if($volume){
    return parseInt($volume.textContent, 10);
  }
};


exports.issue = function($articleMeta){
  if(! $articleMeta) return;

  var $issue = $articleMeta.getElementsByTagName('issue')[0];
  if($issue){
    return parseInt($issue.textContent, 10);
  }
};
...
//TODO improve: take into account title / remove if contains external resources
exports.about = function($articleMeta){
  if(! $articleMeta) return;
