...
function _match(r, type, hrefs){
  ...
  if (r[typeMap[type]] && r[typeMap[type]].length){
    for (var i=0; i < r[typeMap[type]].length; i++){
      ...
      if (mpath && _path2hrefs(mpath, r[typeMap[type]][i].bundlePath).some(function(x){ return hrefs.indexOf(x) >-1; })){
        return true;
      }
    }
  }
...
/**
 * generates all the possible pubmed central @xlink:href from a file
 * path ```mpath```
 */
function _path2hrefs(mpath, isBundlePath){
...
exports._path2hrefs = _path2hrefs;
