...
Jats2Html.prototype.article = function($node) {
  ...
  return [
    ...
    meta.join(''),
    this.map($node, 'main', [ //property of this.pkgIRI targeting this.articleIRI
...
Jats2Html.prototype.body = function($node) {
  ...
  return this.map($node, 'article', attrList);
};
 ...
 */
Jats2Html.prototype._caption = function($node, tagName, mainTypedResource) {
...
Jats2Html.prototype.fig = function($node, htmlToInsertBeforeEnd) {
  ...
  html += this._caption($node, 'figcaption', typedResources.main); //!! <figcaption> HAS to be direct child of <figure>
...
Jats2Html.prototype.tableWrap = function($node) {
  ...
  html += this._caption($node, 'caption', typedResources.main); //also take into account table-wrap-foot
