...
function Jats2Html (ctx, pkg){
  ...
  if(this.pkg.article){
    this.article = this.pkg.article.filter(function(x){ 
...
Jats2Html.prototype.getAttr = function($node, attrList){
  ...
  attrList = attrList || [{name: 'id', value: $node.getAttribute('id')}, {name: 'typeOf', value: this.getSectionRdfType($node)}];
...
function _citations2Html(pkg, id){
  var html = pkg
    
  return ((id)? ('<ol id="'+ id +'">') : '<ol>')  + html + '</ol>';
};
