...
//TODO escape "<" and  ">" in textContent

//TODO table-wrap-foot <-> schema.org/Comment
...
Jats2Html.prototype._caption = function($node, tagName){
  ...
  if ($label || $caption || $tableWrapFoot){
    ...
    if ($label || $title){
      ...
      if ($label){
        html += this.openingTag($label, 'cite', [
          {name: 'id', value: $label.getAttribute('id')},
          {name: 'property', value: 'schema:alternateName'}
        ]);
      ...
      if ($title){
        html += this.openingTag($title, 'h3', [
          {name: 'id', value: $title.getAttribute('id')},
    ...
    if ($caption){
      ...
      if ($ps && $ps.length){
        html += '<div property="schmea:' + ((tagName === 'figcaption')? 'caption': 'description') + '">';
...
Jats2Html.prototype.fig = function($node){
  ...
  if (typedResources.main && typedResources.main.value){
    var rdfRel = {
      'ImageObject': 'schema:image',
      'VideoObject': 'schema:video',
      'AudioObject': 'schema:audio'
    }[ Array.isArray(typedResources.main.value['@type']) ? typedResources.main.value['@type'][0] : typedResources.main.value['@type'] ] || 'schema:image';

    attrList.push({name: 'rel', value: rdfRel});
  }

  var html = this.openingTag($node, 'figure', attrList);

  html += this.openingTag(null, 'div', [
    {name: 'resource', value: (typedResources.main && typedResources.main.value && typedResources.main.value['@id']) ? (packageJsonld.BASE + typedResources.main.value['@id']) : undefined },
    {name: 'typeof', value:  _type((typedResources.main && typedResources.main.value && typedResources.main.value['@type']) || 'ImageObject') }
  ]);

  if (typedResources.main && typedResources.main.value && typedResources.main.value.name){
    html += '<meta property="schema:name" content="' + typedResources.main.value.name + '" />';
  }
    ...
    typedResources
      ...
      .map(function(x){
        return this.selfClosingTag(null, 'img', [
          ...
          {name: 'property', value: 'schema:thumbnailUrl'}
        ]);
      }, this)
  ).join('');
