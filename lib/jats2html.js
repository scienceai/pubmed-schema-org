...
function Jats2Html (ctx, pkg){
  this.pkg = (pkg)? packageJsonld.linkPackage(clone(pkg)) : {};
  this.ctx = ctx || {};
...
Jats2Html.prototype.article = function($node){
  ...
  var meta = (this.pkg.keywords || [])
    .map(function(x){
      return '<meta property="schema:keyword" content="' + x + '">';
    });
  ...
  return [
    '<!DOCTYPE html>',
    '<html typeof="schema:' + (this.mainArticle['@type'] || 'Article' ) + '">',
    '<head>',
    '<meta charset=utf-8 />',
...
Jats2Html.prototype._pkgMeta2Html = function(){
  ...
  if(this.mainArticle.headline){
    html += '<h1 property="schema:headline">' + this.mainArticle.headline + '</h1>';
  }

  var datePublished = this.mainArticle.datePublished || this.pkg.datePublished;
  if(datePublished){
    html += '<span property="datePublished" content="' + datePublished + '">' + (new Date(datePublished)).toDateString() + '</span>';
  }
  ...
  if(authors.some(function(x){return x;})){
    html += '<ul>';
    html += authors.map(this._personLi('author', 'contributor'), this).join('');
    html += '</ul>';
  }
  ...
  if(accountablePerson){
    ...
    html += '<ul>';
    html += accountablePerson.map(this._personLi('accountablePerson'), this).join('');
    html += '</ul>';
  }
  ...
  if(editor){
    ...
    html += '<ul>';
    html += editor.map(this._personLi('editor'), this).join('');
    html += '</ul>';
  }
 ...
 */
Jats2Html.prototype._personLi = function(prop0, prop){

  prop = prop || prop0;

  return function(person, i){
    var li = '';

    if(person){
      li += '<li property="' + ((i===0) ? prop0: prop) + '" typeof=" '+ person['@type'] + '" >';
      if(person.email){
        li += '<a property="email" href="'+ person.email +'">';
      }

      if(person['@type'] === 'Person'){
        li += ['givenName', 'familyName'].filter(function(x){return x in person;}).map(function(p){
          return '<span property="' + p + '">' + person[p] + '</span> ';
        }).join(' ');
      } else if(person.name){ //works for Organization
        li += '<span property="name">' + person.name + '</span> ';
      }
      ...
      if(person.affiliation){
        li += '<ul property="affiliation">';
        li +=person.affiliation.map(function(affiliation){
          var ulli = '<li  typeof="' + affiliation['@type'] + '">';
          ulli += '<span property="description">' + affiliation.description + '</span>';
          ulli += '</li>';
          return ulli;
        }).join('');
        li += '</ul>';
...
//TODO fix RDFa / structured abstract
Jats2Html.prototype.abstract = function($node){
  var attrList = [
    {name: 'id', value: $node.getAttribute('id')},
    {name: 'property', value: 'schema:abstract'},
...
Jats2Html.prototype._abstractPart = function($sec){
  ...
  var attrList = [
    {name: 'id', value: $sec.getAttribute('id')},
    {name: 'typeof', value: 'schema:Abstract'},
...
Jats2Html.prototype.copyrightHolder = function($node){
  var html = '<div property="schema:copyrightHolder">';

  html += this.selfClosingTag($node, 'meta', [
    { name: 'property', value: 'schema:name' },
    { name: 'content', value: tools.cleanText($node.textContent) }
  ]);
...
Jats2Html.prototype.license = function($node){
  ...
  var href = $node.getAttribute('xlink:href');
  if(href){
    attrList.push({ name: 'content', value: href }); //TODO ? use <link rel="license" href="..." /> instead of tagging <section> Cf. https://developer.mozilla.org/en-US/docs/Web/HTML/Link_types
    html = this.map($node, 'section', attrList);
  } else { //add a wrapper div for the schematext property
    attrList.push({ name: 'typeof', value: 'CreativeWork' });

    html = this.openingTag($node, 'section', attrList);
...
Jats2Html.prototype.licenseP = function($node){
  return this.map($node, 'p');
};
...
Jats2Html.prototype._citations2Html = function(){
  ...
  var html = (this.mainArticle.citation || []).map(function(ref){
    ...
    li += this.openingTag(null, 'li', [
      ...
      {name: 'typeof', value: (ref['@type'])? ('schema:' + ref['@type']) : undefined},
    ...
    if(ref.isPartOf){
      ...
      if(periodical && (periodical.name || periodical.alternateName)){
        periodicalHtml = '<span property="isPartOf" typeof="Periodical">';
        if(periodical.alternateName){
          periodicalHtml += '<span property="alternateName">' + periodical.alternateName + '</span>';
          if(periodical.name){
            periodicalHtml += '<meta property="name" content="' + periodical.name + '" />';
          }
        } else {
          periodicalHtml += '<span property="name">' + periodical.name + '</span>';
        }
        periodicalHtml += '</span>';
      }

      if(volume && volume.volumeNumber !== undefined){
        volumeHtml = '<span property="isPartOf" typeof="PublicationVolume">';
        if(periodicalHtml){
      ...
      if(issue && issue.issueNumber !== undefined){
        issueHtml = '<span property="isPartOf" typeof="PublicationIssue">';
...
Jats2Html.prototype._typedResources = function($node){
  ...
  //get all the <graphic>, <media>, <supplementary-material> with directt @xlink:href.
  ...
  ['dataset', 'sourceCode', 'image', 'audio', 'video', 'article'].forEach(function(type){
    (this.pkg[type] || []).forEach(function(r){
      ...
      if(hrefId && jats._match(r, type, [hrefId])){
        mainResource = {
          type: type,
          value: r
        };
