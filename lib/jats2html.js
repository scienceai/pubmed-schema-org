...
Jats2Html.prototype.getAttr = function($node, attrList) {
  ...
  if ($node && !attrList.some(function(x) {return x.name === 'attrList';})) {
    id.push({name: 'id', value: $node.getAttribute('id')});
  }
...
Jats2Html.prototype.getSectionRdfType = function($node) {
  ...
  //TODO check https://github.com/DrInventor/ontologies
  var deo = {
    'intro': 'deo:Introduction',
    'acknowledgement': 'deo:Acknowledgements',
    'discussion': 'sro:Discussion',
    'material': 'deo:Materials',
    'method': 'deo:Methods',
    'result': 'deo:Results',
    'conclusion': 'sro:Conclusion',
    'reference':'doco:ListOfReferences'
  };

  //synonyms
  deo.introduction = deo.intro;
  deo.bibliography = deo.reference;
...
Jats2Html.prototype.article = function($node) {
  ...
  return [
    ...
    '<head>',
    '<meta charset="utf-8" />',
    (this.pkg.headline) ? ('<title>' + this.pkg.headline + '</title>') : '',
    (this.pkg.keywords) ? ('<meta name="keywords" content="' + this.pkg.keywords.join(', ') + '" />') : '',
    '</head>',
    '<body prefix="' + prefix.join(' ') + '" resource="' + this.pkgIri + '" typeof="' + _type(this.pkg['@type'] || 'Article') + '" >',
    meta.join(''),
    this.map($node, 'article'),
    '</body>',
    '</html>'
  ].join('');
...
Jats2Html.prototype._pkgMeta2Html = function() {
  ...
  if (this.pkg.headline) {
    html += '<h1 property="schema:headline" about="' + this.pkgIri + '" >' + this.pkg.headline + '</h1>';
  }

  var datePublished = this.pkg.datePublished || this.pkg.datePublished;
  if (datePublished) {
    html += '<span property="schema:datePublished" content="' + datePublished + '" about="' + this.pkgIri + '" >' + (new Date(datePublished)).toDateString() + '</span>';
  }

  var authors = [ this.pkg.author || this.pkg.author ].concat(this.pkg.contributor || this.pkg.contributor);
  ...
  var accountablePerson = this.pkg.accountablePerson || this.pkg.accountablePerson;
  ...
  var editor = this.pkg.editor || this.pkg.editor;
  if (editor) {
 ...
 */
Jats2Html.prototype._caption = function($node, tagName, attrList, resourcesMain) {
  ...
  if ($label || $caption || $tableWrapFoot) {
    ...
    if(resourcesMain && resourcesMain.value && resourcesMain.value.name){
      html += '<meta property="schema:name" content="' + resourcesMain.value.name + '" />';
    }
    ...
    if ($label || $title) {
      ...
      if ($label) {
        html += this.openingTag($label, 'cite', [
          {name: 'property', value: resourcesMain && 'schema:alternateName'} //only if resourcesMain (if value === undefined => no property)
        ]);
      ...
      if ($title) {
        html += this.openingTag($title, 'h3', [
          {name: 'property', value: resourcesMain && 'schema:headline'}
        ]);
    ...
    if ($caption) {
      ...
      if ($ps && $ps.length) {
        if(resourcesMain) {
          html += '<div property="schema:' + ((tagName === 'figcaption' || resourcesMain['@type'] === 'ImageObject' || resourcesMain['@type'] === 'VideoObject')? 'caption': 'description') + '">';
        }
    ...
    if ($tableWrapFoot) {
      html += this.tableWrapFoot($tableWrapFoot, resourcesMain);
    }
...
Jats2Html.prototype.tableWrapFoot = function($node, resourcesMain) {
  ...
  if (resourcesMain) {
    attrList.push(
      ...
      {name: 'about', value: ('ldr:' + resourcesMain['@id'])}
    );
  }
  ...
  if ($label || $title) {
    ...
    if ($label) {
      html += this.map($label, 'cite', [
        {name: 'property', value: resourcesMain && 'schema:alternateName'}
      ]);
    }
...
Jats2Html.prototype.fig = function($node, htmlToInsertBeforeEnd) {
  ...
  var resources = this._resources($node);
  ...
  if (resources.main) {
    attrList.push(
      {name: 'about', value: this.pkgIri},
      {name: 'rel', value: 'schema:hasPart'}
    );
  }

  var html = this.openingTag($node, 'figure', attrList);

  html += this._link(resources);
  html += this._thumbnail(resources.all);

  //!! <figcaption> HAS to be direct child of <figure>
  var rdfResourceAttrList = [
    {name: 'resource', value: resources.main['@id'] && ('ldr:' + resources.main['@id']) },
    {name: 'typeof', value:  _type(resources.main['@type'] || 'ImageObject') }
  ];
  html += this._caption($node, 'figcaption', rdfResourceAttrList, resources.main);
...
Jats2Html.prototype.tableWrap = function($node) {
  ...
  if (resources.main) {
    html += this.openingTag(null, 'table', [
      {name: 'resource', value: resources.main['@id'] && ('ldr:' + resources.main['@id']) },
      {name: 'typeof', value:  _type(resources.main['@type'] || 'Dataset') }
    ]);
  } else {
    html += '<table>';
  }

  html += this._caption($node, 'caption', null, resources.main); //also take into account table-wrap-foot
...
Jats2Html.prototype.supplementaryMaterial = function($node) {
  ...
  var rdfResourceAttrList = [
    {name: 'resource', value: resources.main['@id'] && ('ldr:' + resources.main['@id']) },
    {name: 'typeof', value:  _type(resources.main['@type'] || 'CreativeWork') }
  ];
  html += this._caption($node, 'div', rdfResourceAttrList, resources.main);
...
Jats2Html.prototype._citations2Html = function() {
  ...
  var html = (this.pkg.citation || []).map(function(ref) {
    var li = '';
...
/**
 * return one or several resources from this.pkg relevant for the $node.
 */
Jats2Html.prototype._resources = function($node) {

  var resources = [], mainResource;

  //match resource with name === id
  var id = $node.getAttribute('id');
  var parts = this.pkg.hasPart || [];
  ...
  //get all the <graphic>, <media>, <supplementary-material>, <inline-supplementary-material> with direct @xlink:href.
  //Note we only consider the immediate child of $node
  var mtags = ['graphic', 'media', 'supplementary-material', 'inline-supplementary-material'];
  ...
  parts.forEach(function(r){
    if (jats._match(r, hrefs)) {
      resources.push(r);
    }
    if (!mainResource && hrefId && jats._match(r, [hrefId])) {
      mainResource = r
    }
  });
  ...
  return {
    all: resources,
    main: mainResource || ((resources.length === 1)? resources[0] : undefined)
  };
...
/**
 * ```resourcesAll``` is crated by this._resources($node).all
 */
Jats2Html.prototype._thumbnail = function(resourcesAll) {

  var aboutAndPaths = [];
  resourcesAll.forEach(function(r){
    if (r.thumbnail) {
      var thumbs = Array.isArray(r.thumbnail) ? r.thumbnail : [r.thumbnail];
      thumbs.forEach(function(t){
        if (t.filePath) {
          aboutAndPaths.push({about: r['@id'], path: t.filePath});
        } else if (t.encoding) {
          var encs = Array.isArray(t.encoding) ? t.encoding : [t.encoding];
          for (var i=0; i<encs.length; i++){
            if(encs[i].filePath) {
              aboutAndPaths.push({about: r['@id'], path: encs[i].filePath});
              break;
            }
          }
        }
  ...
  return _.uniq(
    aboutAndPaths.map(function(x) {
      return this.selfClosingTag(null, 'img', [
        {name: 'about', value: (x.about && 'ldr:' + x.about)},
        {name: 'src', value: this.ctx[x.path]}
      ]);
    }, this)
  ).join('');
...
Jats2Html.prototype._link = function(resources) {

  return _.uniq(
    resources.all.filter(function(x){return x!== resources.main;})
      .map(function(x) {
        return this.selfClosingTag(null, 'link', [
          {name: 'about', value: this.pkgIri},
          {name: 'rel', value: 'schema:hasPart'},
          {name: 'href', value: 'ldr:' + x['@id']}
        ]);
      }, this)
  ).join('');
