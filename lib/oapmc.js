...
function getPkg(pmcid, ldpm, root, opts, callback){
  ...
  readTargzFiles(root, function(err, xml, files, mainArticleName, license){
    ...
    files2resources(ldpm, root, meta, files, mainArticleName, function(err, resources){
      ...
      _inline2imgBase64(files, meta.inline, function(err, imgBase64){
...
function parseXml(xml, pmcid, opts){
  ...
  if($articleMeta){
    ...
    if($altTitle){
      meta.alternativeHeadline = tools.cleanText($altTitle.textContent);
    }
  ...
  if(sourceOrganization.length){
    meta.sourceOrganization = sourceOrganization;
  }
...
function _getRef($ref){
  ...
  if($mixedCitation){
    ...
    var jsDate = _getDate($mixedCitation);
...
function _getTextExcludingTagNames($node, tagNameToExclude){
  var txt = '';
  
  Array.prototype.forEach.call($node.childNodes, function($el){
    if(tagNameToExclude.indexOf($el.tagName) === -1){
      if($el.nodeType === 3){
        txt += $el.textContent;
      } else if ($el.nodeType === 1){
        txt += _getTextExcludingTagNames($el, tagNameToExclude);
      }
    }
  });
 ...
 * Note that some resources will be merged together
 */ 
...
function meta2pkg(resources, meta, mainArticleName, pmcid){
  ...
  if(meta.sourceOrganization){
    pkg.sourceOrganization = meta.sourceOrganization;
  }
  ...
  if(mainArticle){
    ...
    if(meta.alternativeHeadline){
      mainArticle.alternativeHeadline = meta.alternativeHeadline;
    }      
    if (meta.abstract){
...
function _inline2imgBase64(files, inlines, callback){
  ...
  async.each(inlines, function(href, cb){

    //reconstruct all possible extension (including '') and prioritize .jpg
    var mpath;
    for(var i = 0; i< allExts.length; i++){
      var ind = basenames.indexOf(href + allExts[i]);
      if(ind > -1){
        mpath = files[ind] + allExts[i];
        if(allExts[i] === '.jpg'){
          break;
        }
      }
    ...
    fs.readFile(mpath, function(err, buffer){
      if(err) return cb(err);
