...
function oapmc(pmcid, opts, callback){
  ...
  _fetchTargz(pmcid, that, function(err, rootTargz){
    ...
    _pkg(pmcid, that, rootTargz, opts, function(err, pkg, root, files, inlines, $doc){
      if (err) return callback(err);
      if (opts.html) {
        _html(pkg, root, files, inlines, $doc, callback);
      } else {
        callback(err, pkg);
      }
    });
 ...
 */
function _fetchTargz(pmcid, dcat, callback){
  ...
  request(uri, function(error, response, oaContentBody){
    ...
    dcat.log(response.statusCode, uri);
    ...
    try {
      ...
      return callback(new Error('could not get PMC tar.gz'));
    }

    temp.mkdir('__dcatTmp', function(err, rootTargz) {
      if (err) return callback(err);
      ...
      dcat.log('GET', tgzUri, 'ftp');
      c.on('ready', function() {
        c.get(puri.path, function(err, stream) {
          if (err) return callback(err);
          dcat.log(200, tgzUri, 'ftp');
...
function _pkg(pmcid, dcat, rootTargz, opts, callback){
  ...
  _readTargzFiles(rootTargz, function(err, xml, files, mainArticleName, license){
    ...
    _files2resources(dcat, rootTargz, files, inlines, mainArticleName, function(err, resources){
      if (err) return callback(err);
      ...
      async.each(dnode, function(d, cb){
        ...
        d.filePath = uid + '.html';
        var absPath = path.join(dcat.root, d.filePath);
        files.push(absPath);
        ...
        if (opts.pmid){
          _addPubmedAnnotations(pkg, dcat, opts.pmid, opts, function(err, pkg){
            if (err) return callback(err);
            return callback(null, pkg, dcat.root, files, inlines, $doc)
          });
        } else {
          callback(null, pkg, dcat.root, files, inlines, $doc);
        }
...
function _addPubmedAnnotations(pkg, dcat, pmid, opts, callback){
  if (arguments.length === 4){
    callback = opts;
    opts = {};
  }

  pubmed.call(dcat, pmid, opts, function(err, pubmedPkg){
    if (err) return callback(err);

    if (pubmedPkg.about){
      pkg.about = pubmedPkg.about;
    }
...
function _files2resources(dcat, rootTargz, files, inlines, mainArticleName, callback){
  ...
  async.eachSeries(compressedFiles, function(f, cb){
    ...
    //mv files and dir to dcat.root
    function _newPath(p){
      return path.join(dcat.root, path.relative(rootTargz, p));
    };

    async.each(files.concat(dirs), function(p, cb){
      var newPath = _newPath(p);
      var dirnameNewPath = path.dirname(newPath);
      if(dirnameNewPath === dcat.root){
        fs.rename(p, newPath, cb);
      } else {
        mkdirp(dirnameNewPath, function(err){
          if (err) return cb(err);
          fs.rename(p, newPath, cb);
        });
      }
      ...
      if (err) return callback(err);
      dcat.wrap((files.concat(dirs)).map(_newPath), callback);
    });
