...
function parseXml(xml, pmid){
  ...
  if($PubmedArticle){
    ...
    if($Journal){
      ...
      var periodical = pubmedPeriodical($Journal);     
      ...
      if($Pagination){
        ...
        if($MedlinePgn){
          ...
          if(matchPage){ //fix ranges like 1199-201 or 12-9
            ...
            article.pageStart = tools.parseInt(pageStart);
            article.pageEnd = tools.parseInt(pageEnd);
          } else {
    ...
    if(periodical && periodical.alternateName){
      pkgName.push(periodical.alternateName.replace(/ /g, '-').replace(/\W/g, '').toLowerCase());
    }
