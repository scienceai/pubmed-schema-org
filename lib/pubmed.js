...
function parseXml(xml, pmid){
  ...
  if($PubmedArticle){
    ...
    if($ArticleTitle){
      article.headline = tools.cleanText($ArticleTitle.textContent).replace(/^\[/, '').replace(/\]\.*$/, ''); //remove [] Cf http://www.nlm.nih.gov/bsd/licensee/elements_descriptions.html#articletitle
    }
    ...
    if($Journal){
      ...
      //pages (bibo:pages or bibo:pageStart and bibo:pageEnd e.g <Pagination> <MedlinePgn>12-9</MedlinePgn>)
      ...
      if($Pagination){
        ...
        if($MedlinePgn){
          ...
          if(matchPage){ //fix ranges like 1199-201 or 12-9
            ...
            article.pageStart = parseInt(pageStart, 10);
            article.pageEnd = parseInt(pageEnd, 10);
          } else {
            article.pages = medlinePgn;
          }
        }
      }
